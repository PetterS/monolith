...
int scs_printf_buffer_size = 100000;
int scs_also_print_to_stdout = 1;
char* custom_scs_buffer = NULL;
int custom_scs_buffer_pos = 0;
void custom_scs_printf(const char * format, ...) {
	if (custom_scs_buffer == NULL) {
		custom_scs_buffer = calloc(scs_printf_buffer_size, sizeof(char));
	}

	va_list args;
	va_start(args, format);
	while (1) {
		int available = scs_printf_buffer_size - custom_scs_buffer_pos;
		int written = vsnprintf(custom_scs_buffer + custom_scs_buffer_pos, available, format, args);
		if (written >= available) {
			scs_printf_buffer_size += scs_printf_buffer_size + written;
			custom_scs_buffer = realloc(custom_scs_buffer, scs_printf_buffer_size);
		} else {
			custom_scs_buffer_pos += written;
			break;
		}
	}
	va_end(args);

	if (scs_also_print_to_stdout) {
		va_start(args, format);
		vprintf(format, args);
		va_end(args);
	}
...
const char* get_scs_log() {
	custom_scs_buffer_pos = 0;
	return custom_scs_buffer;
}

...
class KalahaState {
   public:
	...
	KalahaState(short start_seeds_ = 3) : start_seeds(start_seeds_) {
		...
		while (seeds > 0) {
			...
			for (auto m = move + 1; m < num_bins && seeds > 0; ++m) {
				...
				if (seeds == 0) {
					...
					if (lower_bins[m]
					    == 1 /* && upper_bins[opposite] > 0 */) {  // Allow null capture?
						// Capture everything
	...
	double get_result(int current_player_to_move) const {
		...
		double win_amount =
		    0.5 * abs(double(player1_sum - player2_sum)) / (player1_sum + player2_sum);
	...
	void print(ostream& out) const {
		...
		auto print_stores = [&out](short left_store, short right_store) {
			out << '|' << setw(4) << right << left_store << "                        " << setw(4)
			    << right << right_store << "  |" << endl;
		};
   ...
   private:
	void check_invariant() const {
		...
		minimum_core_assert(player1_sum + player2_sum == (num_bins * 2) * start_seeds);
	}

...
#include <minimum/ai/games/connect_four.h>
...
void main_program() {
	...
	minimum::ai::ComputeOptions player1_options, player2_options;
	...
	while (state.has_moves()) {
		...
		if (state.player_to_move == 1) {
			move = minimum::ai::compute_move(state, player1_options);
			state.do_move(move);
		} else {
			if (human_player) {
				while (true) {
				...
				move = minimum::ai::compute_move(state, player2_options);
				state.do_move(move);
			}
		}
	}

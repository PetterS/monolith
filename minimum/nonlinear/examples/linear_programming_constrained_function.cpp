...
struct LinearObjective {
	double c;
	LinearObjective(double c_) : c(c_) {}
...
// Constraint aTx ≤ b.
class LinearConstraintTerm : public Term {
   public:
	std::vector<double> a;
	double b;
...
int main_function() {
	...
	f.add_constraint_term(
	    "sum x_i <= 0", std::make_shared<LinearConstraintTerm>(a1, b), all_x_pointers);
	...
	b = 10;
	f.add_constraint_term(
	    "sum x_i >= -10", std::make_shared<LinearConstraintTerm>(a2, b), all_x_pointers);

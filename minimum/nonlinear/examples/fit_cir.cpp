...
R normpdf(const R& x, const R& mu, const R& sigma) {
	...
	return 1.0 / (sigma * sqrt(2.0 * 3.141592)) * exp(-diff * diff);
}
...
R lognormpdf(const R& x, const R& mu, const R& sigma) {
	R diff = (mu - x) / (sqrt(2.0) * sigma);
	return -log(sigma * sqrt(2.0 * 3.141592)) - diff * diff;
}

struct CIRApproximateLL {
	const std::vector<double>& data;
	CIRApproximateLL(const std::vector<double>& data_in) : data(data_in) {}
...
int main_function() {
	...
	for (size_t i = 0; i < data.size() - 1; ++i) {
		f.add_term(std::make_shared<AutoDiffTerm<SimulateCIR, 3>>(data[i], data[i + 1], M, K),
		           &param[0]);
	}

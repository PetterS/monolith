...
class BALProblem {
   public:
	BALProblem(const std::string& filename) {
		...
		check(result, "Could not load BA problem file.");
	}
	...
	double* mutable_point_for_observation(int i) { return mutable_points() + point_index[i] * 3; }
...
template <typename T>
inline void cross_product(const T x[3], const T y[3], T x_cross_y[3]) {
...
//
template <typename T>
inline void angle_axis_rotate_point(const T angle_axis[3], const T pt[3], T result[3]) {
	T w[3];
	T sintheta;
	T costheta;
	...
	if (theta2 > 0.0) {
		...
		for (int i = 0; i < 3; ++i) {
			result[i] =
			    pt[i] * costheta + w_cross_pt[i] * sintheta + w[i] * (T(1.0) - costheta) * w_dot_pt;
		}
	} else {
...
class SnavelyReprojectionError {
   public:
	...
	template <typename T>
	T operator()(const T* const camera, const T* const point) const {
...
int main(int argc, char** argv) {
	try {
		...
		for (int i = 0; i < bal_problem.number_of_observations(); ++i) {
			...
			auto term = std::make_shared<AutoDiffTerm<SnavelyReprojectionError, 9, 3>>(
		...
		std::cerr << results;
		function.print_timing_information(std::cerr);

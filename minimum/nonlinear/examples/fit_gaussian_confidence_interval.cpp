...
class MuEstimator {
   public:
	...
	struct NegLogLikelihoodUsingPointer {
		double* sample;
		NegLogLikelihoodUsingPointer(double* sample) { this->sample = sample; }
	...
	MuEstimator(size_t data_size) : samples(data_size, 0.0) {
		...
		for (auto& sample : samples) {
			f.add_term(std::make_shared<AutoDiffTerm<NegLogLikelihoodUsingPointer, 1, 1>>(&sample),
			           &mu,
			           &sigma);
		}
	}
	...
	double operator()(const std::vector<double>& new_samples) {
		minimum_core_assert(samples.size() == new_samples.size());
	...
	double get_average_error() const { return accumulated_error / num_optimizations; }
...
std::pair<double, double> compute_interval_simple(const std::vector<T>& samples,
	...
	auto interval =
	    make_pair(-numeric_limits<double>::infinity(), numeric_limits<double>::infinity());
...
int main_function() {
	...
	minimum::core::Timer timer("Generating samples");
	...
	cout << "Average error (compared to closed-form ML-estimator): "
	     << mu_estimator.get_average_error() << endl;
	cout << "Average time per optimization problem: " << 1e6 * timer.get_elapsed_time() / n_samples
	     << " micros." << endl;

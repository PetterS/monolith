...
class FieldsOfExperts {
   public:
	...
	int num_variables() const { return size_ * size_; }
	...
	const std::vector<int>& get_x_delta_indices() const { return x_delta_indices_; }

	const std::vector<int>& get_y_delta_indices() const { return y_delta_indices_; }
...
class FoETerm : public spii::Term {
   public:
	FoETerm(double alpha_, const std::vector<double>& filter_) : alpha(alpha_), filter(filter_) {}

	virtual int number_of_variables() const override { return int(filter.size()); }
...
FieldsOfExperts::FieldsOfExperts(const std::string& filename) : size_{-1}, num_filters_{-1} {

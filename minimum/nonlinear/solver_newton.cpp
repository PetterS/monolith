...
void NewtonSolver::solve(const Function& function, SolverResults* results) const {
	...
	if (use_sparsity && this->factorization_method == FactorizationMethod::MESCHACH) {
		if (this->log_function) {
			this->log_function(
			    "Can not use the Meschach library for sparse problems. Switching to iterative "
			    "factorization.");
		}
	...
	if (use_sparsity) {
		...
		if (this->log_function) {
			...
			char str[1024];
			std::sprintf(str,
			             "H is %tdx%td with %td (%.5f%%) non-zeroes.",
			             sparse_H.rows(),
			             sparse_H.cols(),
			             sparse_H.nonZeros(),
			             100.0 * nnz);
	...
	while (true) {
		...
		if (this->check_exit_conditions(fval,
		                                fprev,
		                                normg,
		                                normg0,
		                                x.norm(),
		...
		if (factorization_method == FactorizationMethod::ITERATIVE) {
			...
			while (true) {
				...
				minimum_core_assert(factorizations <= 100, "Solver::solve: factorization failed.");
			}
			...
			minimum_core_assert(!use_sparsity);
		...
		double alpha = this->perform_linesearch(function, x, fval, g, p, &x2, start_alpha);

		if (alpha <= 1e-15) {
			// Attempt a simple steepest descent instead.
			p = -g;
			alpha = this->perform_linesearch(function, x, fval, g, p, &x2, 1.0);
			if (alpha <= 0) {
				if (this->log_function) {
		...
		if (this->log_function && iter % log_interval == 0) {
			if (use_sparsity) {
				if (iter == 0) {
					this->log_function(
					    "Itr        f        max|g_i|   alpha    fac    tau   min(H_ii)");
				}
				this->log_function(
				...
				if (iter == 0) {
					this->log_function(
					    "Itr        f        max|g_i|   ||H||     det(H)    min(H_ii)  alpha    "
					    "fac");
				}

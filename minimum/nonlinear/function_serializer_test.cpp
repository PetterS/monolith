...
struct NormTwo {
	NormTwo() { global_number_of_terms++; }
...
struct NegLogLikelihood {
	...
	NegLogLikelihood() { this->sample = 0; }
	...
	void read(std::istream& in) { in >> sample; }

	void write(std::ostream& out) const { out << sample; }
};

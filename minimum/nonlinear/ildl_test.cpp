...
#include <minimum/nonlinear/sym-ildl-conversions.h>
...
TEST_CASE("ildl-sym") {
	...
	cerr << "A is " << Alilc.n_rows() << " by " << Alilc.n_cols() << " with " << Alilc.nnz()
	     << " non-zeros." << endl;
	...
	cerr << "L is " << Llilc.n_rows() << " by " << Llilc.n_cols() << " with " << Llilc.nnz()
	     << " non-zeros." << endl;
	cerr << Llilc << endl;
	cerr << "D is " << Dblockdiag.n_rows() << " by " << Dblockdiag.n_cols() << " with "
	     << Dblockdiag.nnz() << " non-zeros." << endl;
	...
	cerr << " S^-1 * P * L * D * L^T * P^T * S^-1 = " << endl << SiPLDLtPtSi << endl;
...
TEST_CASE("ildl-sym-sparse") {
	...
	auto add_element = [&](int i, int j, double v) { triplets.emplace_back(i - 1, j - 1, v); };

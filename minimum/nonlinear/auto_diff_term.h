// Petter Strandmark.
#ifndef MINIMUM_NONLINEAR_AUTO_DIFF_TERM_H
#define MINIMUM_NONLINEAR_AUTO_DIFF_TERM_H
...
// as arg_sizes.
template <int... arg_sizes, typename Functor, typename... Ts>
std::shared_ptr<Term> make_differentiable(Functor&& lambda, Ts... arguments) {
	static_assert(sizeof...(arg_sizes) > 0,
	              "make_differentiable<n0, n1, ...> must at least provide n0.");
	typedef typename std::remove_reference<Functor>::type FunctorClass;
	typedef AutoDiffTerm<FunctorClass, arg_sizes...> TermType;
	return std::make_shared<TermType>(arguments..., std::forward<Functor>(lambda));
}
...
typename std::enable_if<has_write<Functor, std::ostream&>::value, void>::type call_write_if_exists(
    std::ostream& out, const Functor& functor) {
	functor.write(out);
}
template <typename Functor>
typename std::enable_if<!has_write<Functor, std::ostream&>::value, void>::type call_write_if_exists(
...
template <typename Functor>
typename std::enable_if<has_read<Functor, std::istream&>::value, void>::type call_read_if_exists(
    std::istream& in, Functor& functor) {
	functor.read(in);
}
template <typename Functor>
typename std::enable_if<!has_read<Functor, std::istream&>::value, void>::type call_read_if_exists(
    std::istream& in, const Functor& functor) {}

// to_double(x) returns the real part of x, disregarding
// any derivatives.
inline double to_double(double x) { return x; }
inline float to_double(float x) { return x; }
template <typename R>
...
T differentiate_functor(const Functor& functor, const T* x_in, T* df) {
...
class AutoDiffTerm<Functor, D0> : public SizedTerm<D0> {
   public:
	template <typename... Args>
	AutoDiffTerm(Args&&... args) : functor(std::forward<Args>(args)...) {}

	virtual void read(std::istream& in) override { call_read_if_exists(in, functor); }

	virtual void write(std::ostream& out) const override { call_write_if_exists(out, functor); }
	...
	virtual double evaluate(double* const* const variables,
		...
		F<double, D0> f(differentiate_functor<Functor, F<double, D0>, D0>(functor, vars, df));
...
class Functor2_to_1 {
   public:
	Functor2_to_1(const Functor& functor_in) : functor(functor_in) {}
...
//
template <typename Functor, int D0, int D1>
class AutoDiffTerm<Functor, D0, D1> : public SizedTerm<D0, D1> {
   public:
	template <typename... Args>
	AutoDiffTerm(Args&&... args) : functor(std::forward<Args>(args)...) {}
	...
	virtual double evaluate(double* const* const variables,
		...
		F<double, D0 + D1> f(
		    differentiate_functor<Functor21, F<double, D0 + D1>, D0 + D1>(functor21, vars, df));
...
class Functor3_to_1 {
   public:
	Functor3_to_1(const Functor& functor_in) : functor(functor_in) {}
...
//
template <typename Functor, int D0, int D1, int D2>
class AutoDiffTerm<Functor, D0, D1, D2> : public SizedTerm<D0, D1, D2> {
   public:
	template <typename... Args>
	AutoDiffTerm(Args&&... args) : functor(std::forward<Args>(args)...) {}

	virtual void read(std::istream& in) override { call_read_if_exists(in, this->functor); }
...
class Functor4_to_1 {
   public:
	Functor4_to_1(const Functor& functor_in) : functor(functor_in) {}
...
template <typename Functor, int D0, int D1, int D2, int D3>
class AutoDiffTerm<Functor, D0, D1, D2, D3> : public SizedTerm<D0, D1, D2, D3> {
   public:
	template <typename... Args>
	AutoDiffTerm(Args&&... args) : functor(std::forward<Args>(args)...) {}
...
struct DoubleFunctorCaller<Functor, D0, DN...> {
	template <typename... T>
	double call(const Functor& functor, double* const* const variables, T... previous_arguments) {
...
struct DoubleFunctorCaller<Functor> {
	template <typename... T>
	double call(const Functor& functor, double* const* const variables, T... arguments) {
...
struct DualFunctorCaller<Functor, R, D0, DN...> {
	R call(const Functor& functor, double* const* const variables) {
	...
	R call_internal(const Functor& functor,
		...
		return next_caller.call_internal(
		    functor, variables + 1, offset + D0, previous_arguments..., x);
	}
};
...
struct DualGradientExtractor<R, D0, DN...> {
	void extract(R& dual, Eigen::VectorXd* gradient, int offset = 0) {
		for (int i = 0; i < D0; ++i) {
...
struct DualGradientExtractor<R> {
	void extract(R& dual, Eigen::VectorXd* gradient, int offset) {
		// We are done.
	}
};

template <typename Functor, int... D>
class AutoDiffTerm : public SizedTerm<D...> {
	static_assert(sizeof...(D) > 0,
	              "D cannot be empty. Sizes are required when creating AutoDiffTerm.");
   ...
   public:
	...
	virtual double evaluate(double* const* const variables,
	                        ...
	                        std::vector<std::vector<Eigen::MatrixXd>>* hessian) const override {
		minimum::core::check(
		    false, minimum::core::to_string(typeid(*this).name(), ": hessian not implemented."));
		return 0;
	}

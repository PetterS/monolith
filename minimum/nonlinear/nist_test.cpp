...
const bool use_sym_ildl = false;

class NewtonSolverSYM_ILDL : public NewtonSolver {};
...
void run_problem_main(const std::string& filename) {
	...
	for (int start = 0; start < problem.initial_parameters.rows(); ++start) {
		...
		output_file << typeid(Model).name() << " " << typeid(SolverClass).name() << " "
		            << "start: " << start + 1 << " "
		            << (log_relative_error < 4 ? "FAILURE" : "SUCCESS") << " "
		            << "LRE: " << log_relative_error << " "
		            << "Initial cost: " << initial_cost << " "
		            << "Final cost: " << final_cost << " "
		            << "Certified cost: " << problem.certified_cost << std::endl;
...
#define NIST_TEST_END(Category, Problem, n)                                                        \
	...
	TEST_CASE(#Category "/" #Problem, "") {                                                        \
		SECTION("Newton") { run_problem_main<NewtonSolver, Problem, n>("nist/" #Problem ".dat"); } \
		SECTION("NewtonSolverSYM_ILDL") {                                                          \
			if (use_sym_ildl) {                                                                    \
				run_problem_main<NewtonSolverSYM_ILDL, Problem, n>("nist/" #Problem ".dat");       \
			}                                                                                      \
		}                                                                                          \
		SECTION("LBFGS") { run_problem_main<LBFGSSolver, Problem, n>("nist/" #Problem ".dat"); }   \
	}

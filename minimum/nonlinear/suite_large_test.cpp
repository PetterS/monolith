...
class NewtonSolverSymIldl : public NewtonSolver
...
void run_test_main(const std::function<void(std::vector<double>&, Function*)>& create_f,
	...
	solver.log_function = [&information_stream](const std::string& str) {
...
void run_test(const std::function<void(std::vector<double>&, Function*)>& create_f,
	...
	if (test_newton) {
		SECTION("Newton-100", "") { run_test_main<NewtonSolver>(create_f, start, 100); }
		SECTION("Newton-1000", "") { run_test_main<NewtonSolver>(create_f, start, 1000); }
		SECTION("Newton-10000", "") { run_test_main<NewtonSolver>(create_f, start, 10000); }

		SECTION("Newton-SymILDL-100", "") {
			run_test_main<NewtonSolverSymIldl>(create_f, start, 100);
		}
		SECTION("Newton-SymILDL-1000", "") {
			run_test_main<NewtonSolverSymIldl>(create_f, start, 1000);
		}
		SECTION("Newton-SymILDL-10000", "") {
			run_test_main<NewtonSolverSymIldl>(create_f, start, 10000);
		}
	}

	SECTION("LBFGS-100", "") { run_test_main<LBFGSSolver>(create_f, start, 100); }
	SECTION("LBFGS-1000", "") { run_test_main<LBFGSSolver>(create_f, start, 1000); }
	SECTION("LBFGS-10000", "") { run_test_main<LBFGSSolver>(create_f, start, 10000); }

	SECTION("LBFGSB-100", "") { run_test_main<LBFGSBSolver>(create_f, start, 100); }
	SECTION("LBFGSB-1000", "") { run_test_main<LBFGSBSolver>(create_f, start, 1000); }
	SECTION("LBFGSB-10000", "") { run_test_main<LBFGSBSolver>(create_f, start, 10000); }
}

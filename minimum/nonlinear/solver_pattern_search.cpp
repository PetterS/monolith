...
void PatternSolver::solve(const Function& function, SolverResults* results) const {
	...
	if (this->log_function) {
		char str[1024];
		std::sprintf(
		    str, "    end %+10.3e %9.3e %9.3e", fval, std::fabs(fval - fprev), pattern_size);
		this->log_function(str);
	}
}
}  // namespace nonlinear

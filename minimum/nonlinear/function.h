...
#include <minimum/nonlinear/auto_diff_change_of_variables.h>
#include <minimum/nonlinear/change_of_variables.h>
#include <minimum/nonlinear/export.h>
#include <minimum/nonlinear/interval.h>
...
class BeginEndProvider {
   public:
	BeginEndProvider(const T* begin_, const T* end_) : begin_pointer{begin_}, end_pointer{end_} {}
...
class MINIMUM_NONLINEAR_API Function {
	friend class Solver;
   ...
   public:
	...
	void add_variable_with_change(double* variable, int dimension) {
		add_variable_internal(
		    variable, dimension, std::make_shared<AutoDiffChangeOfVariables<Change>>(new Change));
	}

	template <typename Change, typename... Args>
	void add_variable_with_change(double* variable, int dimension, Args&&... args) {
		add_variable_internal(variable,
		                      dimension,
	...
	double evaluate(const Eigen::VectorXd& x, Eigen::VectorXd* gradient) const;
	...
	// Sets and gets box constraints for a variable. A solver can use these if
	// it supports box constraints.
	//
	// For example, the global solver and the L-BFGS-B solver uses these bounds.
	void set_variable_bounds(double* variable, std::vector<Interval<double>>);
	const std::vector<Interval<double>>& get_variable_bounds(double* variable) const;
	// Retrieves all variable bounds indexed in global indexing used for evaluation of gradients.
	std::vector<Interval<double>> get_all_variable_bounds() const;

...
static void eigen_to_lilc(const Eigen::MatrixXd& A, lilc_matrix<double>* Alilc_input) {
	...
	int count = 0;
	Alilc.resize(n, n);
	fill(Alilc.row_first.begin(), Alilc.row_first.end(), 0);
	fill(Alilc.col_first.begin(), Alilc.col_first.end(), 0);
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			...
			if (i != j) Alilc.list[j].push_back(i);
			count++;
		}
	}
...
static void eigen_to_lilc(const Eigen::SparseMatrix<double>& A, lilc_matrix<double>* Alilc_input) {
	minimum_core_assert(Alilc_input);
	lilc_matrix<double>& Alilc = *Alilc_input;
...
static Eigen::MatrixXd lilc_to_eigen(const lilc_matrix<double>& Alilc, bool symmetric = false) {
...
static void lilc_to_eigen(const lilc_matrix<double>& Alilc,
...
static Eigen::DiagonalMatrix<double, Eigen::Dynamic> diag_to_eigen(
    const block_diag_matrix<double>& Ablock) {
...
static Eigen::MatrixXd block_diag_to_eigen(const block_diag_matrix<double>& Ablock) {
...
static void block_diag_to_eigen(const block_diag_matrix<double>& Ablock,
                                Eigen::SparseMatrix<double>* A) {
...
class MyPermutation : public Eigen::PermutationMatrix<Eigen::Dynamic, Eigen::Dynamic, int> {
   public:
...
static void solve_block_diag(block_diag_matrix<double>& B, Eigen::VectorXd* x) {
	using namespace Eigen;

	auto n = B.n_rows();
	minimum_core_assert(B.n_cols() == n);
	...
	for (int i = 0; i < n; i = (onebyone ? i + 1 : i + 2)) {
		...
		if (onebyone) {
			...
			Bblock(1, 1) = B[i + 1];
			minimum_core_assert(Bblock(1, 0) == Bblock(0, 1));
...
static void solve_lower_triangular(const lilc_matrix<double>& Llilc, Eigen::VectorXd* x) {
	// Eigen::SparseMatrix<double> L;
	// lilc_to_eigen(Llilc, &L, false);
	//*x = L.triangularView<Eigen::Lower>().solve(*x);

	auto n = Llilc.n_cols();
	minimum_core_assert(Llilc.n_rows() == n);

	for (int j = 0; j < n; ++j) {
		auto n_elements = Llilc.m_idx[j].size();
		minimum_core_assert(Llilc.m_x[j].size() == n_elements);
...
static void solve_lower_triangular_transpose(const lilc_matrix<double>& Llilc, Eigen::VectorXd* x) {
	Eigen::SparseMatrix<double> L;
	...
	*x = L.triangularView<Eigen::Upper>().solve(*x);
	...
	//	minimum_core_assert(m_x[j].size() == n_elements);
...
//
static void solve_system_ildl(block_diag_matrix<double>& B,
	...
	auto& x = *x_output;
	auto n = lhs.rows();
	minimum_core_assert(B.n_rows() == n);
	minimum_core_assert(B.n_cols() == n);
	minimum_core_assert(Llilc.n_rows() == n);
	minimum_core_assert(Llilc.n_cols() == n);

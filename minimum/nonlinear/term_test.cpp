...
class DDFunc {
   public:
	...
	T operator()(T& o_dfdxdx,
		...
		x[1].diff(1);
		F<T, 2> df[2];
		F<T, 2> f(differentiate_functor<Functor, F<T, 2>, 2>(
		    func, x, df));  // Evaluate function and derivatives
		o_dfdx = df[0].x();
		o_dfdy = df[1].x();
		o_dfdxdx = df[0].d(0);
		o_dfdxdy = df[0].d(1);
...
TEST_CASE("FADBAD/differentiate_functor", "") {
	using namespace std;
	double f, dfdx, dfdy, dfdxdx, dfdxdy, dfdydx, dfdydy;
	double x = 1.3;
	double y = 2;
	f = DDFunc<Func>()(
	    dfdxdx, dfdxdy, dfdydx, dfdydy, dfdx, dfdy, x, y);  // Evaluate function and derivatives
...
class DestructorFunctor {
   public:
	DestructorFunctor(int* counter) { this->counter = counter; }

	~DestructorFunctor() { (*counter)++; }
...
TEST_CASE("AutoDiffTerm/make_differentiable") { auto term1 = make_differentiable<1>(MyFunctor1{}); }
...
class MyFunctor4 {
   public:
	template <typename R>
	R operator()(const R* const x, const R* const y, const R* const z, const R* const w) const {
...
struct WriteFunctor1 {
	...
	void write(std::ostream& out) const { out << "Petter"; }
};
...
template <int... D>
class EmptySizedTerm : public SizedTerm<D...> {
   public:
	virtual double evaluate(double* const* const variables) const override { return 0; }
	virtual double evaluate(double* const* const variables,
...
class MyFunctor_1_1_1_2_2_4_4 {
   public:
	...
	R operator()(const R* const x1,
		...
		return 3 * x1[0] + 5 * x2[0] + 9 * x3[0] + 3 * x4[0] + 4 * x4[1] + 1 * x5[0] + 6 * x5[1]
		       + 4 * x6[0] + 3 * x6[1] + 2 * x6[2] + 2 * x6[3] + 2 * x7[0] + 1 * x7[1] + 8 * x7[2]
		       + 9 * x7[3];
	}
};
...
TEST_CASE("AutoDiffTerm/MyFunctor_1_1_1_2_2_4_4", "") {
	...
	std::vector<double*> variables = {x1, x2, x3, x4, x5, x6, x7};
...
struct DetectCopyFunctor {
	...
	DetectCopyFunctor(double data_) : data{data_} { num_constructions++; }
...
TEST_CASE("Term/IntervalsNotImplemented") {
	AutoDiffTerm<MyFunctor1, 2> term;
	Interval<double> x[2] = {0, 0};
	std::vector<Interval<double>*> variables;
	variables.push_back(x);
	CHECK_THROWS(term.evaluate_interval(variables.data()));
}

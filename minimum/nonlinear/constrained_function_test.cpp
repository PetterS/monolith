...
void test_simple_constrained_function(const std::vector<double> x_start, bool feasible) {
	...
	LBFGSSolver solver;
	solver.log_function = [&log_stream](const string& str) { log_stream << str << endl; };
	...
	function.add_constraint_term(
	    "less than 100", make_shared<AutoDiffTerm<LessThan100, 2>>(), &x[0]);
...
TEST_CASE("feasible_start") { test_simple_constrained_function({0.0, 0.0}, true); }

TEST_CASE("infeasible_start") { test_simple_constrained_function({10.0, 4.0}, false); }
...
TEST_CASE("max_iterations") {
	...
	function.add_constraint_term(
	    "less than 100", make_shared<AutoDiffTerm<LessThan100, 2>>(), &x[0]);
	CHECK(function.constraints().size() == 2);
	CHECK(function.constraints().count("circle") == 1);
	CHECK(function.constraints().count("less than 100") == 1);
...
TEST_CASE("no_readding_constraints") {
	...
	double x[2] = {0, 0};
	function.add_constraint_term("circle", make_shared<AutoDiffTerm<ConstraintTerm, 2>>(), &x[0]);
	CHECK_THROWS(function.add_constraint_term(
	    "circle", make_shared<AutoDiffTerm<ConstraintTerm, 2>>(), &x[0]));
}
...
TEST_CASE("feasible_equality") {
	...
	function.add_equality_constraint_term(
	    "equal to 100", make_shared<AutoDiffTerm<EqualTo100, 1>>(), &x);
...
TEST_CASE("Two_lines_intersect") {
	vector<pair<double, double>> start_values = {{3., 4.}, {10., 20.}, {0., 0.1}, {-4., -9.}};
	...
	for (auto start : start_values) {
		...
		function.add_equality_constraint_term(
		    "Line1", make_shared<AutoDiffTerm<Line, 2>>(4, 1, 0), &x[0]);
		function.add_equality_constraint_term(
		    "Line2", make_shared<AutoDiffTerm<Line, 2>>(-1, -5, 1), &x[0]);

		LBFGSSolver solver;
		stringstream sout;
		solver.log_function = [&sout](const string& str) { sout << str << endl; };
		SolverResults results;
...
TEST_CASE("Always_feasible") {
	...
	function.add_constraint_term(
	    "x[0] less than 100", make_shared<AutoDiffTerm<LessThan100, 2>>(), &x[0]);
...
TEST_CASE("Infeasible_problem") {
	...
	function.add_term(make_shared<AutoDiffTerm<Rosenbrock, 2>>(), x.data());
	function.add_constraint_term(
	    "Negative norm", make_shared<AutoDiffTerm<NormLessThanMinus1, 2>>(), x.data());
	REQUIRE(!function.is_feasible());

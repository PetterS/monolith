   ...
   public:
	NISTProblem(std::string filename) {
		...
		for (int i = 0; i < num_tries; ++i) {
			this->initial_parameters(i, parameter_id) = convert<double>(tokens.at(i + 2));
		}
		final_parameters(0, parameter_id) = convert<double>(tokens.at(2 + num_tries));
		...
		for (int i = 0; i < num_observations; ++i) {
			...
			for (int j = 0; j < num_predictors; ++j) {
				this->predictor(i, j) = convert<double>(tokens.at(j + num_responses));
			}
		}
	}
...
struct FunctionNotImplementedError {
	FunctionNotImplementedError(std::string name) : function_name(name) {}
	std::string function_name;
};

namespace minimum {
namespace nonlinear {
Interval<double> atan2(Interval<double> a, Interval<double> b) {
	throw FunctionNotImplementedError("atan2");
	return 0;
}

Interval<double> atan(Interval<double> a) {
	throw FunctionNotImplementedError("atan");
	return 0;
}
}  // namespace nonlinear
}  // namespace minimum
...
void run_problem_with_global_solver(const std::string& filename) {
	using namespace minimum::nonlinear;
	...
	if (num_variables > 4) {
		WARN("Too large problem: " << num_variables);
		return;
	}
	...
	for (int start = 0; start < problem.initial_parameters.rows(); ++start) {
		...
		Eigen::VectorXd initial_parameters = problem.initial_parameters.row(start);
		...
		for (int i = 0; i < problem.predictor.rows(); ++i) {
			...
			auto term = std::make_shared<IntervalTerm<Model, num_variables>>(x, y);
		...
		GlobalSolver solver;
		solver.function_improvement_tolerance = 1e-2;
		solver.maximum_iterations = 20 * 1000 * 1000;
		...
		for (int i = 0; i < num_variables; ++i) {
			double delta = 2.1 * std::abs(problem.final_parameters(i) - initial_parameters[i]);
			double lower = initial_parameters[i] - delta;
			double upper = initial_parameters[i] + delta;
			if (problem.final_parameters(i) > 0 && initial_parameters[i] > 0) {
				lower = std::max(lower, 1e-1 * initial_parameters[i]);
				lower = std::min(lower, 0.9 * problem.final_parameters(i));
			}

			minimum_core_assert(lower < problem.final_parameters(i));
			minimum_core_assert(problem.final_parameters(i) < upper);
			x_interval.emplace_back(lower, upper);
		}

		sout << "Initial parameters (optimal values):" << std::endl;
		for (int i = 0; i < num_variables; ++i) {
			sout << "  b[" << i << "] = " << initial_parameters[i] << " ("
			     << problem.final_parameters(i) << ")" << std::endl;
		}
		sout << "Initital box = " << to_string(x_interval) << std::endl;

		SolverResults results;
		IntervalVector final_box;
		try {
			final_box = solver.solve_global(function, x_interval, &results);
		} catch (const FunctionNotImplementedError& err) {
			WARN(to_string(err.function_name, " is not implemented for Interval<double>."));
			return;
		} catch (const std::exception& err) {
			function.print_timing_information(sout);
			INFO(sout.str());
			INFO(results);
			INFO(err.what());
			FAIL();
		...
		function.print_timing_information(sout);
		INFO(sout.str());
		INFO(to_string(results));

		const double optimum = problem.certified_cost;
		int num_matching_digits =
		    static_cast<int>(-std::log10(fabs(function.evaluate() - optimum) / optimum));

		INFO("Number of matching digits: " << num_matching_digits);
		INFO(to_string("Final box = ", final_box));
		...
		for (int i = 0; i < num_variables; ++i) {
			...
			log_relative_error = std::min(
			    log_relative_error, std::max(0.0, std::min(kMaxNumSignificantDigits, tmp_lre)));
		}
		INFO("LRE: " << log_relative_error);
...
#define NIST_TEST_END(Category, Problem, n)                                  \
	...
	;                                                                        \
                                                                             \
	TEST_CASE(#Category "/" #Problem, "[!hide]") {                           \
		run_problem_with_global_solver<Problem, n>("nist/" #Problem ".dat"); \
	}
...
NIST_TEST_START(Lanczos1) b[0] * exp(-b[1] * x)
    + b[2] * exp(-b[3] * x)
    + b[4] * exp(-b[5] * x)
...
b[0] * (R(1.0) - R(1.0) / sqrt(R(1.0) + R(2.0) * b[1] * x))
...
b[0] + b[1] * cos(R(2.0 * kPi) * x / R(12.0)) + b[2] * sin(R(2.0 * kPi) * x / R(12.0))
    + b[4] * cos(R(2.0 * kPi) * x / b[3]) + b[5] * sin(R(2.0 * kPi) * x / b[3])

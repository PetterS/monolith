...
class Interval {
   public:
	...
	Interval(const R1& _lower, const R2& _upper) : lower(_lower), upper(_upper) {
		minimum_core_assert(lower <= upper, "(", lower, ", ", upper, ")");
	}

	template <typename R2>
	Interval(R2 value) : lower(value), upper(value) {
		minimum_core_assert(value == value, "(", lower, ", ", upper, ")");
	}
	...
	R get_lower() const { return lower; }

	R get_upper() const { return upper; }

	R length() const { return upper - lower; }
	...
	Interval<R> operator-() const { return {-this->upper, -this->lower}; }

	const Interval<R>& operator+() const { return *this; }
	...
	bool contains(const R& x) { return lower <= x && x <= upper; }

   private:
	R lower, upper;
};

using IntervalVector = std::vector<Interval<double>>;
...
Interval<R> pow(const Interval<R>& base, double exponent) {
	minimum::core::check(base.get_lower() >= 0,
	                     "pow(Interval<R> base, double): base cannot be negative.");
...
Interval<R> pow(const Interval<R>& base, const Interval<R>& exponent) {
	minimum::core::check(base.get_lower() >= 0,
	                     "pow(Interval<R> base, Interval<R>): base cannot be negative.");
	return exp(exponent * log(base));
}

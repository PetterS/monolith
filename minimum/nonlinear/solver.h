...
struct MINIMUM_NONLINEAR_API SolverResults {
	// The exit condition specifies how the solver terminated.
	enum {
		GRADIENT_TOLERANCE,  // Gradient tolerance reached.
	...
	bool exit_success() const {
		return exit_condition == GRADIENT_TOLERANCE || exit_condition == FUNCTION_TOLERANCE
...
MINIMUM_NONLINEAR_API std::ostream& operator<<(std::ostream& out, const SolverResults& results);
...
class MINIMUM_NONLINEAR_API FactorizationCache;

// Used to call Solver::check_exit_conditions.
struct CheckExitConditionsCache;

#ifdef _WIN32
MINIMUM_NONLINEAR_API_EXTERN_TEMPLATE template class MINIMUM_NONLINEAR_API
    std::function<void(const std::string&)>;
MINIMUM_NONLINEAR_API_EXTERN_TEMPLATE template class MINIMUM_NONLINEAR_API
    std::function<bool(const CallbackInformation&)>;
#endif

class MINIMUM_NONLINEAR_API Solver {
   public:
	Solver();
	virtual ~Solver();
	...
	enum { BISECTION, CUBIC } wolfe_interpolation_strategy = CUBIC;
...
class MINIMUM_NONLINEAR_API NewtonSolver : public Solver {
   public:
...
class MINIMUM_NONLINEAR_API LBFGSSolver : public Solver {
   public:
...
// L-BFGS-B. Like L-BFGS but also allows box constraints on the
// variables.
class MINIMUM_NONLINEAR_API LBFGSBSolver : public Solver {
   public:
	// Number of vectors L-BFGS should save in its history.
	int lbfgs_history_size = 10;
...
// more iterations.
class MINIMUM_NONLINEAR_API NelderMeadSolver : public Solver {
   public:
...
class MINIMUM_NONLINEAR_API PatternSolver : public Solver {
   public:
	// Area tolerance. The solver terminates if
	// ||a|| / ||a0|| < tol, where ||.|| is the maximum
	// norm.
...
// arithmetic.
class MINIMUM_NONLINEAR_API GlobalSolver : public Solver {
   public:
	// Use these to obtain the box containing the global solution.
	// The first one requires that all variables in the function
	// have finite box constraints.
	IntervalVector solve_global(const Function& function, SolverResults* results) const;
	// This overload uses custom box constraints.
	IntervalVector solve_global(const Function& function,
	                            const IntervalVector& start_box,
	                            SolverResults* results) const;

	IntervalVector solve_global(const ConstrainedFunction& function,
	                            const IntervalVector& start_box,
	                            SolverResults* results) const;

	// Will just give a point in the box of the global optimum.
	virtual void solve(const Function& function, SolverResults* results) const override;

   private:
	IntervalVector private_solve(const Function& function,

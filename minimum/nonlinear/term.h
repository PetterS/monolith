...
class MINIMUM_NONLINEAR_API Term {
   public:
	virtual ~Term(){};
	...
	virtual Interval<double> evaluate_interval(
	    const Interval<double>* const* const variables) const;
	// Overload these if input/output is required.
	virtual void read(std::istream& in);
	virtual void write(std::ostream& out) const;
};

MINIMUM_NONLINEAR_API std::ostream& operator<<(std::ostream& out, const Term& term);
MINIMUM_NONLINEAR_API std::istream& operator>>(std::istream& in, Term& term);
...
struct IntElements<D0> {
	...
	static int get_position(int pos) { return D0; }
};

template <int... D>
class SizedTerm : public Term {
   public:
	virtual int number_of_variables() const override { return sizeof...(D); }

...
void info_log_function(const std::string& str) { global_string_stream << str << "\n"; }
...
double run_test(double* var, Solver* solver_input = 0) {
	...
	minimum_core_assert(solver != nullptr);
	...
	bool is_RosenbrockFar =
	    functor_name.find("Rosenbrock") != std::string::npos && std::abs(var[0]) > 1e3;
	if (!is_RosenbrockFar) {

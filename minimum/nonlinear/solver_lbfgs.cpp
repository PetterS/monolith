...
void LBFGSSolver::solve(const Function& function, SolverResults* results) const {
	...
	// L-BFGS history.
	std::vector<Eigen::VectorXd> s_data(this->lbfgs_history_size), y_data(this->lbfgs_history_size);
	std::vector<Eigen::VectorXd*> s(this->lbfgs_history_size), y(this->lbfgs_history_size);
	...
	while (true) {
		...
		start_time = wall_time();
		if (iter > 1
		    && this->check_exit_conditions(fval,
		                                   fprev,
		                                   normg,
		                                   normg0,
		                                   x.norm(),
		...
		if (iter > 0) {
			...
			auto y_squared_norm = y[0]->squaredNorm();
			if (std::abs(y_squared_norm) < 1e-100) {
				H0 = std::numeric_limits<double>::quiet_NaN();
			} else {
				H0 = s[0]->dot(*y[0]) / y[0]->squaredNorm();
			}

			// If isinf(H0) || isnan(H0)
			if (H0 == std::numeric_limits<double>::infinity()
			    || H0 == -std::numeric_limits<double>::infinity() || H0 != H0) {
				should_restart = true;
			}
		}
		...
		double restart_test =
		    std::fabs(fval - fprev) / (std::fabs(fval) + std::fabs(fprev) + 1e-100);
		if (iter > 0 && iter % 100 == 0 && restart_test < this->lbfgs_restart_tolerance) {
			should_restart = true;
		}
		...
		if (should_restart) {
			if (this->log_function) {
				...
				if (number_of_restarts <= 10) {
					std::sprintf(
					    str,
					    "Restarting: fval = %.3e, deltaf = %.3e, max|g_i| = %.3e, test = %.3e",
					    fval,
					    std::fabs(fval - fprev),
					    normg,
					    restart_test);
		...
		double alpha_step = this->perform_linesearch(function, x, fval, g, r, &x2, start_alpha);

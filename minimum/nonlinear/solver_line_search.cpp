...
double polynomial_interpolation(std::array<double, 2> x,
	...
	if (inside_sqrt >= 0) {
		...
		auto t = x[maxpos]
		         - (x[maxpos] - x[minpos])
		               * ((df[maxpos] + d2 - d1) / (df[maxpos] - df[minpos] + 2 * d2));
	...
	*scratch = x + alpha * p;
	...
	while (iterations <= max_iterations) {
		...
		if (solver.wolfe_interpolation_strategy == Solver::BISECTION) {
			...
			// Double braces for GCC 4.7 compatibility. Remove later.
			alpha = polynomial_interpolation(
			    {{temp, alpha}}, {{f_prev, f_new}}, {{gtp_prev, gtp_new}}, minStep, maxStep);
		}
	...
	while (!done && iterations <= max_iterations) {
		...
		if (solver.wolfe_interpolation_strategy == Solver::BISECTION) {
			...
			alpha = polynomial_interpolation(bracket, bracket_fval, bracket_gTpval);
		}

		auto max_bracket = std::max({bracket[0], bracket[1]});
		auto min_bracket = std::min({bracket[0], bracket[1]});

		if (std::min({max_bracket - alpha, alpha - min_bracket}) / (max_bracket - min_bracket)
		    < 0.1) {

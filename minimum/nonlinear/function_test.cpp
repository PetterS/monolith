...
class DestructorTerm : public SizedTerm<1> {
   public:
	DestructorTerm(int* counter) { this->counter = counter; }

	~DestructorTerm() { (*counter)++; }

	virtual double evaluate(double* const* const variables) const { return 0; }
...
class DestructorChange {
   public:
	DestructorChange(int* counter) { this->counter = counter; }

	~DestructorChange() { (*counter)++; }
	...
	int x_dimension() const { return 1; }

	int t_dimension() const { return 1; }
...
TEST(Function, evaluate) {
	...
	EXPECT_DOUBLE_EQ(fval,
	                 sin(x[0]) + cos(x[1]) + 1.4 * x[0] * x[1] + 1.0 + log(y[0]) + 3.0 * log(z[0]));
...
TEST(Function, evaluate_x) {
	...
	EXPECT_DOUBLE_EQ(
	    fval, sin(xg[0]) + cos(xg[1]) + 1.4 * xg[0] * xg[1] + 1.0 + log(xg[2]) + 3.0 * log(xg[3]));
}
		...
		for (int b2 = 0; b2 <= 1; b2++) {
			for (int b3 = 0; b3 <= 1; b3++) {
				...
				if (b1 == 1) {
					...
					EXPECT_DOUBLE_EQ(gradient[ind + 0],
					                 2.0 * (cos(xg[ind + 0]) + 1.4 * xg[ind + 1]));
					EXPECT_DOUBLE_EQ(gradient[ind + 1],
					                 2.0 * (-sin(xg[ind + 1]) + 1.4 * xg[ind + 0]));
				}
				if (b2 == 1) {
...
class Mixed3_2 {
   public:
	template <typename R>
	R operator()(const R* const x, const R* const y) const {
		return 9.0 * x[0] * y[0] + 10.0 * x[0] * y[1] + 11.0 * x[1] * y[0] + 12.0 * x[1] * y[1]
...
TEST(Function, evaluate_hessian) {
	...
	EXPECT_DOUBLE_EQ(hessian(2, 2), 2.0 * 3.0 - 14.0 * xg[4] * xg[4] * cos(xg[2] * xg[4]));
	...
	EXPECT_DOUBLE_EQ(hessian(4, 4), 2.0 * 7.0 - 14.0 * xg[2] * xg[2] * cos(xg[2] * xg[4]));
	...
	EXPECT_DOUBLE_EQ(hessian(2, 4),
	                 -14.0 * (sin(xg[2] * xg[4]) + xg[2] * xg[4] * cos(xg[2] * xg[4])));
	EXPECT_DOUBLE_EQ(hessian(4, 2),
	                 -14.0 * (sin(xg[2] * xg[4]) + xg[2] * xg[4] * cos(xg[2] * xg[4])));
...
class Circle {
   public:
	...
	int x_dimension() const { return 2; }
...
TEST(Function, rethrows_error) {
	...
	f1.add_variable(&x, 1);
	for (int i = 1; i < 40; ++i) {
		f1.add_term(std::make_shared<AutoDiffTerm<ThrowsRuntimeError, 1>>(), &x);
	}
	...
	f2.add_variable(&x, 1);
	for (int i = 1; i < 40; ++i) {
		f2.add_term(std::make_shared<AutoDiffTerm<ThrowsCString, 1>>(), &x);
	}
...
struct SimplePolynomial {
	template <typename R>
	R operator()(const R* const x) const {
		return x[0] * x[0] * x[0] * x[0] - 20.0 * x[0] * x[0] * x[0] + 150.0 * x[0] * x[0]
		       - 500.0 * x[0] + 625.0;
	}
};
...
TEST(Function, evaluate_interval) {
	...
	Function f;
	f.add_variable(&x, 1);
	f.add_term(std::make_shared<IntervalTerm<SimplePolynomial, 1>>(), &x);
...
TEST_CASE("variable_bounds") {
	Function f;
	double x;
	double y[2];
	f.add_variable(&x, 1);
	f.add_variable(y, 2);
	f.set_variable_bounds(&x, {{1.0, 2.0}});
	f.set_variable_bounds(y, {{3.0, 4.0}, {5.0, 6.0}});
	CHECK(f.get_variable_bounds(&x) == (std::vector<Interval<double>>{{1.0, 2.0}}));
	CHECK(f.get_variable_bounds(y) == (std::vector<Interval<double>>{{3.0, 4.0}, {5.0, 6.0}}));
	f.set_variable_bounds(y, {});
	CHECK(f.get_variable_bounds(y).empty());
}

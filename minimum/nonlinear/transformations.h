...
class GreaterThanZero {
   public:
	GreaterThanZero(int dimension) { this->dimension = dimension; }
...
class IntervalConstraint {
   public:
	IntervalConstraint(double a, double b) {
		minimum_core_assert(b > a, "Interval: non-positive size");
		this->a = a;
		this->b = b;
	}
...
class Box {
   public:
	Box(int dimension, const double* constraint_lower, const double* constraint_upper) {
		for (int i = 0; i < dimension; ++i) {
			minimum_core_assert(constraint_upper[i] > constraint_lower[i],
			                    "Interval: non-positive size");
		}

...
struct SimplexPoint {
	...
	bool operator<(const SimplexPoint& rhs) const { return this->value < rhs.value; }
};
...
namespace std {
template <>
void swap<minimum::nonlinear::SimplexPoint>(minimum::nonlinear::SimplexPoint& lhs,
                                            minimum::nonlinear::SimplexPoint& rhs) noexcept {
	lhs.x.swap(rhs.x);
...
void NelderMeadSolver::solve(const Function& function, SolverResults* results) const {
